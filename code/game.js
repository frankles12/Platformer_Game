...
//make sure commas are in the correct places
var actorChars = {
    "@": Player
...
function Level(plan) {
    ...
    for (var y = 0; y < this.height; y++) {
        ...
        for (var x = 0; x < this.width; x++) {
            ...
            else if (ch == "y") fieldType = "floater";
...
function Deathcoin(pos) {
    this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
    this.size = new Vector(0.6, 0.6);
    // Make it go back and forth in a sine wave.
    this.wobble = Math.random() * Math.PI * 2;
}
Deathcoin.prototype.type = "deathcoin";
...
var wobbleSpeed = 0
    , wobbleDist = 0.00;
Deathcoin.prototype.act = function (step) {
    this.wobble += step * wobbleSpeed;
    var wobblePos = Math.sin(this.wobble) * wobbleDist;
    this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Level.prototype.playerTouched = function (type, actor) {
    ...
    else if (type == "deathcoin" && this.status == null) {
        this.status = "lost";
        this.finishDelay = 1;
    }
    else if (type == "coin") {
    ...
    else if (type == "teleporter" && this.status == null) {
        this.status = "tele";
        this.finishDelay = 1;
    }
};
...
function runGame(plans, Display) {
    function startLevel(n) {
        ...
        runLevel(new Level(plans[n]), Display, function (status) {
            ...
            else if(status == "tele") startLevel(n);
